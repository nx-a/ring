...
func New(pool *pgxpool.Pool) *Point {
	return &Point{pool: pool,
		crud: storage.NewCrud[domain.Point, uint64](scan, pool,
			"select point_id, bucket_id, external_id, time_zone from point where point_id = $1",
			"select point_id, bucket_id, external_id, time_zone from point where point_id in ($1)",
			"insert into point (bucket_id, external_id, time_zone) values ($1, $2, $3) RETURNING point_id",
			"update point set bucket_id = $2, external_id = $3, time_zone = $4 where point_id = $1",
			"delete from point where point_id = $1",
		)}
}
func scan(row pgx.CollectableRow) (domain.Point, error) {
	var point domain.Point
	err := row.Scan(&point.PointId, &point.BucketId, &point.ExternalId, &point.TimeZone)
	return point, err
}
func (b *Point) Add(point domain.Point) domain.Point {
	point.PointId = b.crud.Add(point.BucketId, point.ExternalId, point.TimeZone)
	return point
}
...
func (b *Point) Update(id uint64, point domain.Point) domain.Point {
	rows := b.crud.Update(id, point.BucketId, point.ExternalId, point.TimeZone)
	log.Info(rows)
	point.PointId = id
	return point
}
...
func (b *Point) GetByBacketId(id uint64) []domain.Point {
	rows, err := b.pool.Query(context.Background(), "select point_id, bucket_id, external_id, time_zone from point where bucket_id = $1", id)
	if err != nil {
		log.Error(err)
		return nil
	}
